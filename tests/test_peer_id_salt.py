"""The peer-ID salt must never take the dashboard down, or become predictable.
...
Two failure modes, pulling in opposite directions:
  ...
  * The obvious way to make that go away — a constant, a default, a skip — is
    the one thing that must not happen. The salt is what stops sha256(ip) being
    invertible over the whole IPv4 space, so an unwritable salt file has to
    degrade to a fresh random salt and a loud warning, never to a known value.

These run the import in a subprocess because the salt is resolved once per
...
# Reports the salt actually in use, so a test can compare two processes.
PROBE = (
    "import ws_server;"
...
def run_probe(salt_file=None, env_salt=None, expect_ok=True):
    env = dict(os.environ)
    ...
    if expect_ok:
        assert proc.returncode == 0, (
            f"importing ws_server failed with salt_file={salt_file}\n"
            f"--- stdout ---\n{proc.stdout}\n--- stderr ---\n{proc.stderr}"
        )
    out = dict(
...
class TestTheSaltIsActuallySecret:
    ...
    def test_an_empty_salt_file_is_not_used_as_the_salt(self, tmp_path):
        ...
        Reading it back as "" would hash every IP unsalted, which is the exact
        reversible scheme the salt file exists to prevent — and it would do so
        silently, because an empty string is a perfectly valid salt as far as
        sha256 is concerned.
        """
        salt_file = tmp_path / "salt.secret"
